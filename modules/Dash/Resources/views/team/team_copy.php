...
jsPlumb.ready(function () {
    ...
    instance.batch(function () {
        ...
        function rec(c,parent){
            for(var x in c){
                 if ('children' in c[x]){
                    rec(c[x].children,c[x].id);
                    s.push('employee'+parent+'-bottom'+'##'+ 'employee'+c[x].id+'-top');
                }else{
                    s.push('employee'+parent+'-bottom'+'##'+ 'employee'+c[x].id+'-top');
                    continue;
                }
        ...
        for (var i in s) {
            if((s.length-1) == i){
                instance.connect({uuids: [s[i].split('##')[0], s[i].split('##')[1]], overlays: overlays, detachable: true, reattach: true});
            }else{
                instance.connect({uuids: [s[i].split('##')[0], s[i].split('##')[1]], overlays: overlays});
            }
        };   
    ...
    <?php foreach ($employees as $employee):?>
        <div class="window" id="employee<?php echo $employee->id;?>">
            <div class="contact-card">
                <a class="pull-left" href="/dash/profile/<?php echo $employee->id;?>">
                    <?php if($employee->gender == 'F'):?>
                        <img class="img-circle" width="40px" src="/images/employees/girl.png" alt="">
                    <?php else:?>
                        <img class="img-circle" width="40px" src="/images/employees/boy.png" alt="">
                    <?php endif;?>
                    </a>
